...
import { useGLTF, useAnimations, Float, AdaptiveDpr, PerformanceMonitor } from "@react-three/drei"
...
function useIsLargeScreen() {
    ...
    useEffect(() => {
      const checkScreenSize = () => {
        // Consider screens 1024px and above as "large screens" (laptops/desktops)
  ...
  function RobotModel() {
    const group = useRef()
    const { scene, animations } = useGLTF('/unstable_robot.glb')
    ...
    useEffect(() => {
      if (animations && animations.length > 0) {
        ...
        if (actions[firstAnimationName]) {
          ...
          setCurrentAnimation(firstAnimationName)
        }
    ...
    useEffect(() => {
      ...
      if (hovered && animations.length > 1) {
        const hoverAnimationName = animations[1]?.name || animations[0].name
        
        if (actions[hoverAnimationName] && currentAnimation !== hoverAnimationName) {
          actions[currentAnimation]?.fadeOut(0.3)
          actions[hoverAnimationName]?.reset().fadeIn(0.3).play()
          setCurrentAnimation(hoverAnimationName)
        }
      } else if (!hovered && currentAnimation !== animations[0].name) {
        const idleAnimationName = animations[0].name
        
        if (actions[idleAnimationName]) {
          actions[currentAnimation]?.fadeOut(0.3)
          actions[idleAnimationName]?.reset().fadeIn(0.3).play()
          setCurrentAnimation(idleAnimationName)
        }
      }
    }, [hovered, actions, animations, currentAnimation])
      ...
      >
        <group ref={group}>
          <primitive
            object={clonedScene}
            scale={hovered ? 1.2 : 1}
            position={[2, -1, 0]} // Positioned to the right side
            rotation={[0, -Math.PI / 6, 0]} // Slightly angled toward the text
  ...
  // Large Screen 3D Canvas Component (Laptops/Desktops Only)
  function Desktop3DCanvas() {
    const [dpr, setDpr] = useState(1)
    const isLargeScreen = useIsLargeScreen()
  
    // Only render on large screens (1024px and above)
    if (!isLargeScreen) return null
  
    return (
      <div className="absolute inset-0 pointer-events-auto z-10 opacity-70">
        <Canvas
          ...
          gl={{
            ...
            alpha: true,
            antialias: true, // Enable antialiasing for better quality on desktop
            stencil: false,
            depth: true
          }}
        >
        ...
        <div className="flex flex-col items-center justify-center min-h-[80dvh] text-center max-w-[75rem] mx-auto">
          ...
          >
            ...
            <motion.div className="relative z-20">
              <motion.h1 
                className="text-[3.5rem] sm:text-[5rem] md:text-[7rem] lg:text-[9rem] xl:text-[11rem] font-black mb-[2rem] bg-gradient-to-r from-white via-gray-100 to-gray-300 bg-clip-text text-transparent leading-[0.85] tracking-tighter"
                initial={{ opacity: 0, y: 40, scale: 0.9 }}
                animate={{ opacity: 1, y: 0, scale: 1 }}
                transition={{ duration: 1.8, ease: [0.4, 0, 0.2, 1], delay: 0.3 }}
              >
                Automate.<br />Accelerate.<br />Achieve.
              </motion.h1>
            </motion.div>

            <motion.p 
              className="text-[1.5rem] sm:text-[2rem] lg:text-[2.5rem] text-white mb-[1.5rem] font-semibold relative z-20"
            ...
            <motion.p 
              className="text-[1.125rem] sm:text-[1.25rem] lg:text-[1.375rem] text-gray-300 mb-[3rem] leading-relaxed max-w-[50rem] mx-auto relative z-20"
            ...
            {/* Stats */}
            <motion.div 
              className="flex flex-col sm:flex-row gap-[2rem] sm:gap-[4rem] mb-[4rem] justify-center relative z-20"
            ...
            {/* CTA Buttons */}
            <motion.div 
              className="flex flex-col sm:flex-row gap-[1.5rem] items-center justify-center relative z-20"
...
useGLTF.preload('/unstable_robot.glb')